...
metadata:
  ...
  source_path: skills/.curated/agent-skills-main/skills/deploy-to-vercel/SKILL.md
---
...
- Nested archive copies were omitted; use the unpacked scripts and references included in this skill.
...
**When to use:** When the Vercel CLI is unavailable or cannot authenticate in the current Codex environment. Try the CLI first, then use the bundled fallback script if authentication is impossible.
...
- Resolve the skill directory from the repository-local `.agents/skills/deploy-to-vercel/` path; do not assume a home-directory installation.
- Check CLI availability and authentication before deployment.
- Request approval before `git push`, production deployment, or any external side effect.
- If sandbox policy blocks the actual network operation, request the required network approval for that command only; do not elevate harmless local checks.
...
If `vercel login` or `vercel deploy` fails with authentication errors, fall back to the repository-local Codex no-auth deploy script.
