...
metadata:
  ...
  source_path: skills/.curated/render-deploy/SKILL.md
---
...
**4. Codex MCP Setup (if MCP is not configured)**

If `list_services()` is unavailable because the Render MCP server is not configured, offer either the Codex MCP setup below (preferred for direct single-service creation) or the CLI fallback.

1. Ask the user to create a Render API key in the Render dashboard. Never ask them to paste the secret into chat or commit it to the repository.
2. Store the key in the local shell environment:
...
4. Restart/reload Codex if needed, verify the configured server with `codex mcp list`, then retry `list_services()`.

For a project-scoped configuration, follow the repository's existing `.codex/config.toml` conventions rather than writing secrets into the file.
