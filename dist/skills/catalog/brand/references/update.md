...
Use `a focused clarification only when essential` to collect:
...
```bash
# Interactive mode
`$brand` — update the current brand system

# With theme hint
`$brand` — update the brand to “Ocean Professional”

# Quick preset
`$brand` — update the brand using a “midnight purple” direction
```
