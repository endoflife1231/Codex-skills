...
If the directory is empty (greenfield), don't pick a framework silently. Ask the user once through the available Codex user-input mechanism, with sensible defaults framed by the brief:
...
If Codex has the `$imagegen` capability, this step is mandatory. **Stop and load [codex.md](codex.md)**. It covers palette generation, mock exploration, the approval loop, mock-fidelity inventory, and asset slicing via the `impeccable_asset_producer` project agent. Follow Steps A-F in that file, then return here for Step 4.
