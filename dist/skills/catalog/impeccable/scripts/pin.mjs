 ...
 *
 * This Codex migration writes only to .agents/skills in the project root.
 */
...
function generatePinnedSkill(command, metadata) {
  const desc = metadata[command]?.description || `Run the Impeccable ${command} workflow.`;

  return `---
name: ${command}
description: "${desc} Use only when the user explicitly invokes $${command} or asks for the Impeccable ${command} workflow."
---
...
Load \`$impeccable\`, route to its \`${command}\` reference, pass along the user's target, and follow that workflow.
`;
}
