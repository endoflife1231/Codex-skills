...
description: Beautifies and generates ASCII art diagrams in markdown files. Use when user says "beautify ascii",
  "fix ascii art", "align ascii", "выровняй разметку", "причеши ascii", "сделай разметку ровной", "fix diagram",
  "clean up ascii", or points to a markdown file with messy ASCII art in code blocks. Also generates new clean ASCII
  art from text descriptions when asked.
metadata:
...
Invoke with `$ascii-art-beautifier`. Use Codex file and shell capabilities rather than Claude-specific tool names. Do not overwrite source files during review-only tasks unless the user explicitly asks for in-place editing.
...
6. **Write** the corrected blocks back. Always write via Python:
   ...
   with open(filepath, 'w') as f:
       f.write(content)
   ```
   The Edit and file-writing capabilitys silently strip trailing spaces from lines. For simple boxes where every line ends with a visible character (`│` or `┘`), this doesn't matter. But for any diagram with nested boxes, connection lines, or padded rows, stripped trailing spaces break uniform line width and misalign borders. Since it's hard to predict which diagrams will need trailing spaces, always use Python — it's safe for all cases.

7. **Validate** by running `python3 .agents/skills/ascii-art-beautifier/validate_ascii_art.py <file>` (located in this skill's directory). Compare error counts before and after. Target: 0 errors. Warnings about "vertical border gap" between separate boxes are expected and OK.
...
```bash
python3 .agents/skills/ascii-art-beautifier/validate_ascii_art.py file.md                  # check all blocks
python3 .agents/skills/ascii-art-beautifier/validate_ascii_art.py file.md --fix             # auto-fix line widths (pad shorter lines)
python3 .agents/skills/ascii-art-beautifier/validate_ascii_art.py file.md --fix-nested      # rebuild nested box alignment (detects outer
                                                        # frame, fixes inner │ positions, writes via Python)
python3 .agents/skills/ascii-art-beautifier/validate_ascii_art.py file.md --block 2         # check only block #2
python3 .agents/skills/ascii-art-beautifier/validate_ascii_art.py file.md --verbose          # show detected boxes and nesting structure
```
...
5. **Always write via Python**: The Edit and file-writing capabilitys strip trailing spaces. This is invisible and breaks uniform line width. Always use `with open(filepath, 'w')` — it preserves every character exactly as written. This is especially critical for nested boxes where inner content lines are shorter than the outer frame.
