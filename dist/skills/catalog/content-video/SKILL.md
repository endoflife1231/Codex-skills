---
name: content-video
description: Video script generation and Remotion composition scaffolding. Use when user wants to create video content,
  storyboards, Remotion scripts, social media shorts, explainer videos, or video scripts.
---
...
1. **Check for brand identity**: Read `.agents/brand/VOICE.md` and `.agents/brand/VISUAL.md` if they exist. Video content requires both voice (for narration/script tone) and visual identity (for colors, fonts, logo animations). If these files do not exist, use sensible defaults and suggest the user run the content-setup skill.
...
- Apply brand voice from `.agents/brand/VOICE.md`
...
- Apply brand colors and fonts from `.agents/brand/VISUAL.md`
...
2. **Brand tokens as constants**: Extract colors, fonts, and spacing from `.agents/brand/VISUAL.md` into a shared `theme.ts` file.
...
**Visual consistency** (from `.agents/brand/VISUAL.md`):
...
**Voice consistency** (from `.agents/brand/VOICE.md`):
