---
name: content-setup
description: Interactive brand onboarding that creates `.agents/brand/` with BRAND.md, VOICE.md, and VISUAL.md.
  Use for `$content-setup`, brand review/update, voice calibration, or when a content skill detects missing brand
  context.
---
...
1. **Explicit invocation**: user runs `$content-setup`, `$content brand show`, `$content brand update`, or `$content-setup`.
2. **Missing brand detection**: any other content skill (e.g., `$content draft`, `$content social`) checks for the `.agents/brand/` directory and it does not exist. That skill should hand off here before proceeding.
3. **Voice recalibration**: user drops writing samples into `.agents/brand/voice-samples/` and runs `$content-setup` to re-derive voice attributes.
...
| `$content brand show` | Display current brand configuration from `.agents/brand/BRAND.md`, `.agents/brand/VOICE.md`, `.agents/brand/VISUAL.md` |
| `$content brand update` | Interactive modification of existing brand files |
| `$content-setup` | Analyze writing samples in `.agents/brand/voice-samples/` and update `.agents/brand/VOICE.md` |
...
If writing samples exist in `.agents/brand/voice-samples/`, analyze them for patterns: sentence length, vocabulary level, punctuation habits, rhetorical devices, tone shifts.
...
|---|---|---|
| `.agents/brand/BRAND.md` | `templates/BRAND.md.template` | Core identity, audience, platforms, content pillars |
| `.agents/brand/VOICE.md` | `templates/VOICE.md.template` | Voice attributes, example sentences, vocabulary, rhythm |
| `.agents/brand/VISUAL.md` | `templates/VISUAL.md.template` | Color palette, typography, logo rules, visual principles |

Create the `.agents/brand/` directory if it does not exist. If files already exist, confirm before overwriting.
...
1. Check that `.agents/brand/BRAND.md`, `.agents/brand/VOICE.md`, and `.agents/brand/VISUAL.md` exist.
...
When the user runs `$content-setup`:

1. Check for `.agents/brand/voice-samples/` directory.
...
5. Update `.agents/brand/VOICE.md` with the analysis, merging with any existing manual entries.

---

## Error Handling

- If the user abandons mid-flow, save progress to `.agents/brand/.setup-draft.json` so they can resume later.
