---
name: seo-technical
description: 'Technical SEO audit across 9 categories: crawlability, indexability, security, URL structure, mobile,
  Core Web Vitals, structured data, JavaScript rendering, and IndexNow protocol. Use when user says "technical SEO",
  "crawl issues", "robots.txt", "Core Web Vitals", "site speed", or "security headers".
...
python3 .agents/vendor/claude-seo/scripts/agent_ux_check.py https://example.com --json
```
...
without scoring, use `python3 .agents/vendor/claude-seo/scripts/render_page.py <url> --a11y-tree --json`.
...
If Google API credentials are configured, use `python3 .agents/vendor/claude-seo/scripts/pagespeed_check.py <url> --json` for real PSI + CrUX field data (replaces lab-only CWV estimates), `python3 .agents/vendor/claude-seo/scripts/crux_history.py <url> --json` for 25-week CWV trends, and `python3 .agents/vendor/claude-seo/scripts/gsc_inspect.py <url> --json` for real indexation status per URL.
