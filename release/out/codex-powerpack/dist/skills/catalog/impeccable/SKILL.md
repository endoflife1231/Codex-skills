---
name: impeccable
description: Audit, shape, polish, harden, clarify, optimize, or iteratively improve production product interfaces. Use for dashboards, app shells, tools, components, forms, settings, onboarding, empty/error states, accessibility, UX copy, responsive behavior, design-system extraction, anti-pattern detection, and live browser iteration. Prefer design-taste-frontend for brand-led landing pages or portfolios and huashu-design for prototypes, decks, motion, or narrated media. Not for backend-only work.
---

Designs and iterates production-grade frontend interfaces. Real working code, committed design choices, exceptional craft.

## When to use and pack boundaries

- This is the production product-UI and audit workflow in this pack.
- Route brand-led landing pages, portfolios, and campaigns to `$design-taste-frontend` unless the user explicitly invokes Impeccable.
- Route HTML prototypes, decks, motion, video, and narrated explainers to `$huashu-design`.
- Never enable the design hook automatically. Run the `hooks` command only when the user explicitly asks, preview the resulting project-local hook, and remind them that Codex requires trust review through `/hooks`.
- Spawn the bundled custom agents only with explicit user permission. If permission is absent or declined, complete the asset or source-edit step in the parent session.

## Workflow

Run Setup, route to the matching command reference, make the requested production change or assessment, and complete that reference's verification and delivery contract. Do not load unrelated command references.

## Setup

You MUST do these steps before proceeding:

1. Run `node .agents/skills/impeccable/scripts/context.mjs` once per session. If the request names or implies a file, route, or app inside a monorepo, infer the concrete path and run `node .agents/skills/impeccable/scripts/context.mjs --target <path>` instead. If you've already seen its output in this conversation, do not re-run it. The script either prints the project's PRODUCT.md (and DESIGN.md when present) as a markdown block, or reports `NO_PRODUCT_MD`. For substantial new design work, follow `reference/init.md` first. For a narrow audit, critique, copy fix, accessibility fix, or explicitly scoped edit, derive temporary context from the requested surface and proceed; offer documentation afterward instead of blocking the task. This pack does not self-update Impeccable independently—updates must come through a reviewed pack integration.
...
**Pin** creates a standalone Codex shortcut so `$<command>` invokes `$impeccable <command>` directly. **Unpin** removes it. This migrated script writes only to the project's `.agents/skills` directory.
