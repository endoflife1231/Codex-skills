...
description: 'Build a brand-led editorial strategy for the content-studio workflow: audience beliefs, content pillars,
  channel mix, idea backlog, and briefs. Use when the user wants editorial positioning tied to `.agents/brand`;
  use `$content-strategy` for the broader marketing/SEO content strategy.'
---
...
1. **Check for existing brand context**: Read `.agents/brand/VOICE.md` and `.agents/brand/VISUAL.md` if they exist. Strategy work must align with the established brand identity. If no brand files exist, the strategy phase is a good time to define them -- suggest the user run content-setup first, or build foundational voice/visual decisions into the strategy output.
