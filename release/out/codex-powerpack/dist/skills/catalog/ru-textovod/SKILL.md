---
name: ru-textovod
description: Checks Russian text for spelling, punctuation, and grammar via Textovod.com API. Use for autocorrection,
  spelling, or punctuation checks after writing/editing Russian text. Requires user-provided credentials; not for
  stylistic editing (use `$ru-editor`).
metadata:
...
Invoke with `$ru-textovod`. Use Codex file and shell capabilities rather than Claude-specific tool names. Do not overwrite source files during review-only tasks unless the user explicitly asks for in-place editing.
...
Launch Codex from a process that has these variables set; see `.agents/CODEX_INTEGRATIONS.md`. Never print or commit secret values.
