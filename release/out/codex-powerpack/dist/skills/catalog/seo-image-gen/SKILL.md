---
name: seo-image-gen
description: Generate and edit SEO image assets such as OG/social previews, blog heroes, schema images, product
  visuals, infographics, banners, thumbnails, and favicons. Uses an optional Gemini/nanobanana MCP integration or
  bundled direct-API scripts; verify capability, cost, and approval before calls.
license: MIT
metadata:
  author: AgriciDaniel
  version: 2.2.0
...
- **Agent** (`.codex/agents/seo_image_gen.toml`): Audit-only analyst spawned during `$seo-audit` to assess existing OG/social images and produce a generation plan (never auto-generates)
...
| Error | Resolution |
|-------|-----------|
| MCP not configured | Run `references/CODEX-SETUP.md` |
...
| Extension not installed | Show install instructions: `references/CODEX-SETUP.md` |
