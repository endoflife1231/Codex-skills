---
name: content-blog
description: Long-form blog post generation with brand voice integration and anti-slop enforcement. Use when user
  wants to write a blog post, article, tutorial, case study, opinion piece, deep dive, or listicle.
---
...
1. **Check for brand voice**: Read `.agents/brand/VOICE.md` if it exists. This file defines the brand's tone, vocabulary, formality level, and signature patterns. If it does not exist, use sensible defaults (clear, direct, conversational-professional) and suggest the user run the content-setup skill to configure their voice.
...
- Brand voice from `.agents/brand/VOICE.md` (see `voice-integration.md`)
...
1. Read `.agents/brand/VOICE.md` and internalize the voice attributes
