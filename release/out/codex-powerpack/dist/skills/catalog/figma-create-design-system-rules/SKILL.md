...
description: Generates custom design system rules for the user's codebase. Use when user says "create design system rules", "generate rules for my project", "set up design rules", "customize design system guidelines".
metadata:
  codex_migration: '2026-06-27'
  source_path: skills/.curated/figma-create-design-system-rules/SKILL.md
---
...
- Verify the required MCP server is configured before using MCP-only steps; otherwise use the documented local/CLI fallback or report the missing dependency.
...
Use `AGENTS.md` in the project root for repository-wide rules. Use a nested `AGENTS.override.md` only when a subtree needs more specific Figma implementation rules.
...
### Step 4: Save Rules for Codex

Write the generated rules to the project-root `AGENTS.md` under a clearly named section such as `## Figma implementation rules`.

- If `AGENTS.md` already exists, merge the new section without deleting unrelated project instructions.
- Keep repository-wide conventions in the root file.
- When rules apply only to a subtree, place a narrower `AGENTS.override.md` in that subtree instead of bloating the root file.
- Keep instructions concrete, testable, and scoped; link to longer local documentation rather than copying large references into `AGENTS.md`.
- Re-read the final instruction chain from repository root to the target directory and remove contradictions.

After saving, Codex will load the applicable project instructions for future Figma implementation tasks.
