...
Use `$planning-and-task-breakdown` to create persistent TODO tasks via the parent Codex session's explicit task checklist. Break down into:
...
Present ideas to user via `a focused clarification only when essential` for approval before designing.
...
3. **Randomly invoke ONE of:** `$ui-ux-pro-max` OR `$frontend-ui-engineering` — for layout, hierarchy, visual balance. Pick one at random each run for design variety.
...
Invoke `$browser-testing-with-devtools` with instructions to:
