---
name: seo-cluster
description: 'SERP-based semantic topic clustering for content architecture planning. Groups keywords by actual
  Google SERP overlap (not text similarity), designs hub-and-spoke content clusters with internal link matrices,
  and generates interactive visualizations. Optionally executes content creation if `$content-blog` is installed. Use
  when user says "topic cluster", "content cluster", "semantic clustering", "pillar page", "hub and spoke", "content
  architecture", "keyword grouping", or "cluster plan".
...
| `$seo-cluster execute` | Execute plan: create content via `$content-blog` or output briefs |
...
instead of WebSearch for SERP data. Run `python3 .agents/vendor/claude-seo/scripts/dataforseo_costs.py check serp_organic_live_advanced --count N`
...
```
Test: Does `.agents/skills/content-blog/SKILL.md` exist?
```

**If `$content-blog` is installed:**
...
5. For each post, invoke `$content-blog` with cluster context:
...
**If `$content-blog` is not installed:**
...
4. Inform user: "Install [`$content-blog`](https://github.com/AgriciDaniel/`$content-blog`)
...
| `cluster-briefs/` | Content briefs (if no `$content-blog`) |
...
| "`$content-blog` not installed" | Execute attempted without blog skill | Generate content briefs instead; suggest installation |
...
- All URLs fetched via `python3 .agents/vendor/claude-seo/scripts/render_page.py --mode auto` (SPA-aware SSRF protection via `url_safety`)
