---
name: content
description: Content studio with brand consistency. Use this skill whenever the user wants to create content — blog
  posts, social media (X/Twitter, LinkedIn, Reddit, HN), presentations (Slidev/Reveal/Spectacle), infographics,
  image prompts, video scripts (Remotion), editorial calendars, or content audits. Also triggers on brand setup,
  voice profiling, content repurposing, and content strategy. Covers any request involving writing, publishing,
  visual content creation, or content planning. Even if the user doesn't say "content" explicitly — if they're asking
  to write a blog post, create slides, draft a thread, plan a content calendar, or check content quality — use this
  skill.
---

# Content Studio — Main Router

## Codex content-studio conventions

- Invoke the narrow skill directly (`$content-blog`, `$content-linkedin`, and so on) or use `$content` as a router. The root `$content` router is supported, but direct narrow skills are preferred when the task is already clear.
- Brand context is project-scoped under `.agents/brand/`: `BRAND.md`, `VOICE.md`, `VISUAL.md`, and `voice-samples/`.
- Shared runtime resources live under `.agents/vendor/content-skills/`. Verify optional Node, Python, browser, image, video, or presentation dependencies before use; do not install them or call paid services without approval.
- Custom content agents are available as `.codex/agents/content_*.toml`, but Codex spawns subagents only on an explicit user request. Otherwise run the workflow inline.
...
├── .agents/brand/                     ← shared brand source of truth
...
├── .agents/vendor/content-skills/scripts/  ← shared utilities
├── .agents/vendor/content-skills/styles/   ← shared styles
└── .agents/vendor/content-skills/assets/   ← shared assets
```
...
The `.agents/brand/` directory is the single source of truth for all content generation. Three files form the core:

- **.agents/brand/BRAND.md** — Who you are. Company/personal name, tagline, mission, target audience segments, positioning statement, competitors, key differentiators. Every sub-skill reads this to understand context.

- **.agents/brand/VOICE.md** — How you sound. Tone dimensions (e.g., formal vs. casual, technical vs. accessible), vocabulary preferences, banned words/phrases, sentence rhythm guidelines, humor policy, jargon stance. Includes per-platform overrides (LinkedIn might be more formal than Twitter).

- **.agents/brand/VISUAL.md** — How you look. Primary and secondary color palettes, typography scale, logo usage rules, image style preferences, layout principles. Used by presentation, infographic, and image sub-skills.

Sub-skills must never hard-code brand assumptions. Always read from `.agents/brand/` at generation time.
...
|---|---|---|
| `setup` | `.agents/skills/content-setup/SKILL.md` | Initialize brand identity. Walks the user through defining BRAND.md, VOICE.md, and VISUAL.md via an interactive questionnaire. |
| `brand show` | `.agents/skills/content-setup/SKILL.md` | Display the current brand configuration in a readable summary. |
| `brand update` | `.agents/skills/content-setup/SKILL.md` | Modify specific brand attributes without re-running full setup. |
| `voice` | `.agents/skills/content-setup/SKILL.md` | Analyze writing samples to extract and codify voice characteristics into VOICE.md. |
| `strategy` | `.agents/skills/content-editorial-strategy/SKILL.md` | Develop a content strategy: audience analysis, topic pillars, channel mix, publishing cadence. |
| `plan` | `.agents/skills/content-editorial-strategy/SKILL.md` | Create a content plan for a specific time period with topics, formats, and deadlines. |
| `brief` | `.agents/skills/content-editorial-strategy/SKILL.md` | Generate a detailed content brief for a single piece: angle, audience, key points, CTA, SEO targets. |
| `blog` | `.agents/skills/content-blog/SKILL.md` | Write a long-form blog post. Handles outlining, drafting, SEO metadata, and revision. |
| `twitter` | `.agents/skills/content-twitter/SKILL.md` | Create X/Twitter content: single tweets, threads, quote tweets, reply strategies. |
| `linkedin` | `.agents/skills/content-linkedin/SKILL.md` | Write LinkedIn posts, articles, and engagement content. Handles formatting for the platform's algorithm. |
| `reddit` | `.agents/skills/content-reddit/SKILL.md` | Draft Reddit posts and comments. Adapts tone per subreddit norms. |
| `hn` | `.agents/skills/content-hackernews/SKILL.md` | Write Hacker News submissions and comments. Emphasizes technical substance and avoids promotional tone. |
| `presentation` | `.agents/skills/content-presentation/SKILL.md` | Build slide decks in Slidev, Reveal.js, or Spectacle. Generates content, speaker notes, and styling. |
| `infographic` | `.agents/skills/content-infographic/SKILL.md` | Design data visualizations and infographic layouts with structured data and visual direction. |
| `image` | `.agents/skills/content-image/SKILL.md` | Generate detailed image prompts for AI image generators (DALL-E, Midjourney, Stable Diffusion). Brand-aligned style direction. |
| `video` | `.agents/skills/content-video/SKILL.md` | Write Remotion video scripts: scene structure, narration, motion graphics direction, timing. |
| `calendar` | `.agents/skills/content-calendar/SKILL.md` | Build and manage editorial calendars. Track content status, deadlines, and publishing schedules. |
| `repurpose` | `.agents/skills/content-repurpose/SKILL.md` | Adapt existing content across platforms. Takes one piece and produces variants for multiple channels. |
| `audit` | `.agents/skills/content-audit/SKILL.md` | Score content against quality criteria: brand alignment, readability, SEO, engagement potential. |
| `score` | `.agents/skills/content-audit/SKILL.md` | Alias for audit. Quick quality score on a piece of content. |
...
Look for `.agents/brand/BRAND.md` in the skill pack root.

### Step 2a: Brand Exists

If `.agents/brand/BRAND.md` exists:

1. Read `.agents/brand/BRAND.md` for core identity.
2. Read `.agents/brand/VOICE.md` if it exists. Apply voice rules to all text generation. If the sub-skill targets a specific platform, check for platform-specific overrides within VOICE.md.
3. Read `.agents/brand/VISUAL.md` if it exists and the sub-skill produces visual output (presentation, infographic, image).
4. Check `.agents/brand/voice-samples/` for reference material if doing voice-sensitive work.
5. Apply all brand constraints as hard requirements, not suggestions.

### Step 2b: No Brand Configuration

If `.agents/brand/BRAND.md` does not exist:
...
1. **Strategy phase** — Invoke `.agents/skills/content-editorial-strategy/SKILL.md` to define the series: theme, number of installments, angle per installment, target audience, success metrics.

2. **Creation phase** — Invoke `.agents/skills/content-blog/SKILL.md` for each installment. Pass the series plan as context so each post maintains narrative continuity.

3. **Repurpose phase** — Invoke `.agents/skills/content-repurpose/SKILL.md` for each completed post. Generate platform variants based on the user's channel preferences.
...
2. Pass brand context to every agent. Each agent must independently read from `.agents/brand/` — do not assume shared state between agents.
