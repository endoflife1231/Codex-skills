...
metadata:
  ...
  source_path: skills/.curated/pixijs-scene-sprite/SKILL.md
---
...
Each variant's constructor options are documented in its sub-reference file (`references/{variant}.md`). All variants also accept the `Container` options (`position`, `scale`, `tint`, `label`, `filters`, `zIndex`, etc.) — see `../pixijs-scene-core-concepts/references/constructor-options.md`.
