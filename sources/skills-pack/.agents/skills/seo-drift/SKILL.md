---
name: seo-drift
description: 'SEO drift monitoring: capture baselines of SEO-critical elements, detect changes, and track regressions
  over time. Git for SEO — baseline, diff, and track changes to your on-page SEO. Use when user says "SEO drift",
  "baseline", "track changes", "did anything break", "SEO regression", "compare SEO", "before and after", "monitor
...
2. Fetch page via `.agents/vendor/claude-seo/scripts/fetch_page.py`
3. Parse HTML via `.agents/vendor/claude-seo/scripts/parse_html.py`
4. Optionally fetch CWV via `.agents/vendor/claude-seo/scripts/pagespeed_check.py` (use `--skip-cwv` to skip)
5. Hash HTML body and schema content (SHA-256)
6. Store snapshot in SQLite

**Execution:**
```bash
python3 .agents/vendor/claude-seo/scripts/drift_baseline.py <url>
python3 .agents/vendor/claude-seo/scripts/drift_baseline.py <url> --skip-cwv
```
...
**Execution:**
```bash
python3 .agents/vendor/claude-seo/scripts/drift_compare.py <url>
python3 .agents/vendor/claude-seo/scripts/drift_compare.py <url> --baseline-id 5
python3 .agents/vendor/claude-seo/scripts/drift_compare.py <url> --skip-cwv
```
...
```bash
python3 .agents/vendor/claude-seo/scripts/drift_report.py <comparison_json_file> --output drift-report.html
```
...
**Execution:**
```bash
python3 .agents/vendor/claude-seo/scripts/drift_history.py <url>
python3 .agents/vendor/claude-seo/scripts/drift_history.py <url> --limit 10
```
...
- **All URL fetching** goes through `.agents/vendor/claude-seo/scripts/fetch_page.py` which enforces SSRF protection
