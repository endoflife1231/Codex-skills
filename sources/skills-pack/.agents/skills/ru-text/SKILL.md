---
name: ru-text
description: 'Use when writing, editing, or reviewing Russian-language text, or when user mentions ru-text. Covers
  typography, info-style, editorial, UX writing, business correspondence, AI-text cleanup. Auto-activates on Russian
  text output.
...
Invoke with `$ru-text`. For scoring or check-only behavior, state the mode in the prompt. The skill may activate implicitly for Russian writing, but an explicit user style request overrides defaults. Review-only requests must not overwrite files.
...
`$ru-text (ru-score mode)` — text quality score (0–10, 5 dimensions).
...
If the path is not resolved, search: ``.agents/skills/ru-text/references/scoring.md`` and use the parent directory.
