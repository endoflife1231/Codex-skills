...
> **Note on scope.** Skills below live in this `marketingskills` repo. A few references point to optional tools from adjacent Codex integration catalogs (e.g., `vercel:agent-browser`, `compound-engineering:diagram-maker`) — substitute equivalents if not installed. When a plan references a skill or tool that isn't available, fall back to the underlying tactic and call it out in Section 13's open decisions.
