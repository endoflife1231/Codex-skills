---
name: seo-backlinks
description: 'Backlink profile analysis: referring domains, anchor text distribution, toxic link detection, competitor
  gap analysis. Works with free APIs (Moz, Bing Webmaster, Common Crawl) and DataForSEO extension. Use when user
  says backlinks, link profile, referring domains, anchor text, toxic links, link gap, link building, disavow, or
  backlink audit.'
license: MIT
metadata:
  author: AgriciDaniel
...
2. **Moz API** (free signup): `python3 .agents/vendor/claude-seo/scripts/backlinks_auth.py --check moz --json`
3. **Bing Webmaster** (free signup): `python3 .agents/vendor/claude-seo/scripts/backlinks_auth.py --check bing --json`
...
Run `python3 .agents/vendor/claude-seo/scripts/backlinks_auth.py --check --json` to detect all sources at once.
...
**Moz API:** `python3 .agents/vendor/claude-seo/scripts/moz_api.py metrics <url> --json` → Domain Authority, Page Authority, Spam Score, linking root domains, external links.

**Common Crawl:** `python3 .agents/vendor/claude-seo/scripts/commoncrawl_graph.py <domain> --json` → in-degree (referring domain count), PageRank, harmonic centrality.
...
**Moz API:** `python3 .agents/vendor/claude-seo/scripts/moz_api.py anchors <url> --json`

**Bing Webmaster:** `python3 .agents/vendor/claude-seo/scripts/bing_webmaster.py links <url> --json` (extract anchor text from link details)
...
**Moz API:** `python3 .agents/vendor/claude-seo/scripts/moz_api.py domains <url> --json` → domains with DA scores

**Common Crawl:** `python3 .agents/vendor/claude-seo/scripts/commoncrawl_graph.py <domain> --json` → top referring domains (domain-level, no authority scores)
...
**Moz API:** Spam Score from `python3 .agents/vendor/claude-seo/scripts/moz_api.py metrics <url> --json` (1-17% scale, >11% = high risk)

**Verification Crawler:** `python3 .agents/vendor/claude-seo/scripts/verify_backlinks.py --target <url> --links <file> --json` (verify suspicious links still exist)
...
**Moz API:** `python3 .agents/vendor/claude-seo/scripts/moz_api.py pages <domain> --json`
...
**Bing Webmaster (unique!):** `python3 .agents/vendor/claude-seo/scripts/bing_webmaster.py compare <url1> <url2> --json` — the only free tool with built-in competitor comparison

**Moz API:** Compare DA/PA between domains via `python3 .agents/vendor/claude-seo/scripts/moz_api.py metrics <url> --json` for each
...
**Verification Crawler:** For known links, verify current status with `python3 .agents/vendor/claude-seo/scripts/verify_backlinks.py`
...
| DataForSEO unavailable | Extension not installed | Read `.agents/skills/seo-dataforseo/references/DATAFORSEO-SETUP.md` |
